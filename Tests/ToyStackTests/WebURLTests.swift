...
@Suite struct WebURLTests {
    ...
    @Test func parsesPath() {
        let url = WebURL("https://example.com/some/path")
        #expect(url.path == "/some/path")
    }

    @Test func defaultPortHTTPS() {
        let url = WebURL("https://example.com/")
        #expect(url.port == 443)
    }
    ...
    @Test func customPort() {
        let url = WebURL("https://example.com:8080/")
    ...
    @Test func toStringOmitsDefaultHTTPPort() {
        let url = WebURL("http://example.com/path")
        #expect(url.toString() == "http://example.com/path")
    }

    @Test func toStringIncludesCustomPort() {
        let url = WebURL("http://example.com:8080/path")
    ...
    @Test func resolveAbsoluteURL() {
        let base = WebURL("https://example.com/a/b")
